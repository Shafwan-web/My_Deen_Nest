...
const PaymentHistory = () => {
  ...
  let filteredData = paymentHistoryData.filter((row) => {
    ...
    const matchesProcessedBy =
      filters.processedBy === "" || row.processedBy === filters.processedBy;

    return matchesSearch && matchesStatus && matchesProcessedBy;
  });
          ...
          <div className="relative" ref={filterRef}>
            ...
            {showFilterMenu && (
              <div className="absolute right-0 mt-2 w-64 bg-white border border-gray-200 rounded shadow-md z-10 p-4">
                <h3 className="font-semibold text-sm mb-3">Filter History</h3>
                ...
                <div className="mb-2">
                  ...
                  >
                    ...
                    {[...new Set(paymentHistoryData.map((s) => s.status))].map(
                      (status) => (
                ...
                <div className="mb-2">
                  ...
                  >
                    <option value="">All</option>
                    {[
                      ...new Set(paymentHistoryData.map((s) => s.processedBy)),
                    ].map((person) => (
      ...
      <table className="mt-4 w-full">
        <thead>
          <tr className="border-b border-[#F2F2F2] text-primary text-sm">
            ...
            {visibleColumns.staffCount && (
              <th
                className="cursor-pointer"
                onClick={() => handleSort("staffCount")}
              >
                Staff Count {renderSortIcon("staffCount")}
              </th>
            )}
            ...
            {visibleColumns.processedBy && (
              ...
              >
                Processed By {renderSortIcon("processedBy")}
              </th>
            )}
          ...
          {currentRecords.length > 0 ? (
            currentRecords.map((row, index) => (
              ...
              >
                ...
                {visibleColumns.processedDate && <td>{row.processedDate}</td>}
                {visibleColumns.totalAmount && (
                  <td>₹{row.totalAmount.toLocaleString()}</td>
                )}
                {visibleColumns.staffCount && <td>{row.staffCount}</td>}
                ...
                {visibleColumns.processedBy && <td>{row.processedBy}</td>}
